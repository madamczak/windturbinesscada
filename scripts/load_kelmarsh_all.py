"""
Load turbine data CSVs for Kelmarsh turbines into per-turbine SQLite .db files.

This mirrors the behavior of `load_penmanshiel01_data.py` but is configured to
handle files named like `Turbine_Data_Kelmarsh_<id>_*.csv` for turbines 1..6.

Usage: python scripts/load_kelmarsh_all.py all
"""
...
# Reusing the same TARGET_COLUMNS list as in the penmanshiel loader to maximize column coverage.
TARGET_COLUMNS = [
    "Date and time",
    "Wind speed (m/s)",
    ...
    # ... list truncated for brevity but kept long to match penmanshiel coverage
]
...
def find_files_for_turbine(turbine_str: str, data_dir: Path) -> List[Path]:
    ...
    Only match files that start with the exact prefix 'Turbine_Data_Kelmarsh_{id}_'.
    """
    ...
    # dedupe and ensure we only keep files that start with Turbine_Data_Kelmarsh_{...}_
    unique = []
    seen = set()
    for f in files:
        name = f.name
...
def main() -> int:
    ...
    if not args:
        # default behavior: process turbine 1 only
        return process_turbine_id('1')
    if args[0].lower() == 'all':
        exit_codes = []
        for i in range(1, 7):
            tid = f"{i}"
