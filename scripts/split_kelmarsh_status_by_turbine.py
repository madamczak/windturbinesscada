#!/usr/bin/env python3
"""
Split kelmarsh_all_status.db into per-turbine tables turbine_1..turbine_6 in a
new DB file data/sqlitedbs/kelmarsh_status_by_turbine.db. Logs each step.

Usage:
  python scripts/split_kelmarsh_status_by_turbine.py \
      --src data/sqlitedbs/kelmarsh_all_status.db \
      --dst data/sqlitedbs/kelmarsh_status_by_turbine.db

Assumes the combined/status table is named "Kelmarsh Status" (the script will
...
def detect_table(conn: sqlite3.Connection, preferred_name: str = 'Kelmarsh Status') -> str | None:
    cur = conn.cursor()
    ...
    for n in names:
        ln = n.lower()
        if 'kelmarsh' in ln and 'status' in ln:
            return n
    for n in names:
...
def split_status_db(src: Path, dst: Path, turbines=range(1,7)):
    print('Source DB:', src)
    ...
    try:
        ...
        try:
            ...
            # Create turbine_unknown for rows not matching 1..6 or NULL/empty
...
def main(argv=None):
    ...
    parser.add_argument('--src', default='data/sqlitedbs/kelmarsh_all_status.db')
    parser.add_argument('--dst', default='data/sqlitedbs/kelmarsh_status_by_turbine.db')
